...
لا ترسل صور المنتجات تلقائياً أبداً. إذا لم يطلب العميل الصورة صراحةً، اسأله أولاً بشكل ذكي ومختصر مثل: "تحب أرسللك صورته؟" ولا ترسلها إلا بعد موافقته أو طلبه المباشر (كأن يقول: أرسل صورة، ورّني شكله، شكله كيف، صورة، بغيت أشوفه). النظام هو من يتولى إرسال الصورة فعلياً بعد موافقته؛ أنت فقط اسأل أو أكّد أنك سترسلها. لا ترسل نفس صورة نفس المنتج مرتين في المحادثة نفسها.
...
function buildSystemPrompt() {
  ...
  parts.push(
    ...
    "4) إذا قال العميل «نفس هذا الرقم» لرقم التواصل → اقبله فوراً واعتبر رقم واتساب هو رقم التواصل. لا تسأله مرة ثانية.\n" +
    "5) لا تسجّل الطلب ولا تؤكده أبداً قبل أن تعرف: المنتجات + الكمية + الاسم + رقم التواصل + استلام/توصيل + الوقت" + (needsPayment ? " + طريقة الدفع" : "") + " (والعنوان إن كان توصيلاً).\n" +
    "6) بعد اكتمال كل المعلومات، أرسل رداً واحداً نهائياً بهذا الشكل بالضبط لأن النظام يعتمد على هذا التأكيد لتسجيل الطلب:\n" +
    "   «تأكيد الطلب ✅\n   • المنتجات: …\n   • الكمية: …\n   • الاسم: …\n   • رقم التواصل: …\n   • النوع: توصيل/استلام\n   • الوقت: …\n" +
    (needsPayment ? "   • طريقة الدفع: …\n" : "") +
    "   • العنوان: … (إن كان توصيلاً)\n   • ملاحظات: …\n   شكراً لك، طلبك مسجّل.»\n" +
    "7) لا تكرر تأكيد نفس الطلب في نفس المحادثة. إن سأل العميل بعد التأكيد عن شيء آخر، اعتبره طلباً جديداً فقط إذا صرّح صراحة بإضافة/طلب جديد.\n" +
    "8) في كل خطوة كن ودوداً ومختصراً وذكياً — لا تعامل العميل كنموذج تعبئة."
  );
  ...
  // 📸 قاعدة الصور — لا يرسل البوت صوراً بدون طلب صريح
  parts.push(
    "\n=== 📸 قاعدة الصور ===\n" +
    "ممنوع اقتراح إرسال الصورة أكثر من مرة لنفس المنتج، وممنوع إرسالها تلقائياً. إذا كان يبدو أن العميل يريد رؤية شكل المنتج، اسأله سؤالاً واحداً واضحاً: «تحب أرسللك صورته؟» ثم انتظر الرد. إن قال نعم/طلبها/قال (صورة، شكله، ورّني) → أرسل الصورة مرة واحدة فقط ولا تكررها.",
  );
...
// يستدعى فقط بعد أن يرد البوت، ويحتاج نص العميل الأصلي — لا نرسل صور بلا طلب
async function maybeSendProductImage(phone, chatId, userText, replyText) {
  ...
  // طلب صريح من العميل فقط (وليس من كلام البوت). لو ما طلب — لا شيء.
  const userN = normalize(userText);
  const explicitImageRequest = /(صور|صوره|صورة|شكله|شكلها|ورّني|ورني|ابغى اشوف|ابي اشوف|أبي اشوف|اريد اشوف|اريد أشوف|بغيت اشوف|picture|photo|image|pic)/i.test(userN);
  if (!explicitImageRequest) return;

  // ابحث عن المنتج المذكور في نص العميل أولاً، ثم في رد البوت كاحتياط
  let match = null;
